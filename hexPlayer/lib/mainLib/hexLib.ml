...
let randomPlay (avLines : int array) (avCol : int array array) : int * int =
  let p = avLines.(0) in
  (* print_int p;
     print_string " "; *)
  assert (p != 0);
...
let auxMoveScoreMM (game : hexGame) (curD : int)
    (bestPlay : hexGame -> int -> position * int) ((i, j) : int * int) :
    (int * int) * int =
  play game i j;
  let s = ref 0 in
...
let minMax (maxDepth : int) (heur : hexGame -> int) (game' : hexGame) :
    position * int =
...
let auxMoveScoreAB (game : hexGame) (curD : int)
    (bestPlay : hexGame -> int -> int -> int -> position * int) (alpha : int)
    (beta : int) ((i, j) : int * int) : (int * int) * int =
  play game i j;
  let s = ref 0 in
  (if isWinningPos game 1 then s := max_int
   else if isWinningPos game (-1) then s := min_int
...
let alphaBeta (maxDepth : int) (heur : hexGame -> int) (game' : hexGame) :
    position * int =
  let game = gameCopy game' in
...
let indicePos ((i, j) : position) (n : int) : int =
  if inboundPos n i j then (i * n) + j
  else if (i, j) = (-1, -1) || (i, j) = (-1, n) then n * n
  else if (i, j) = (n, -1) || (i, j) = (n, n) then (n * n) + 1
  else failwith (Printf.sprintf "unknown position %d, %d" i j)

let posIndice (player : int) (i : int) (n : int) : position =
  if i < n * n then (i / n, i mod n)
  else if i = n * n then if player = 1 then (-1, -1) else (-1, n)
  else if player = 1 then (n, n)
  else (n, -1)
...
let hashGame (game : hexGame) : int =
  ...
  for k = 0 to (n * n) - 1 do
    ...
    hash :=
      (3 * !hash)
      + if game.gameSet.(i).(j) = -1 then 2 else game.gameSet.(i).(j)
  done;
  !hash

let alphaBetaHashTabl (maxDepth : int) (heur : hexGame -> int) (game' : hexGame)
    : position * int =
  let cpt = ref 0 in
  let game = gameCopy game' in
...
let minMaxHashtbl (maxDepth : int) (heur : hexGame -> int) (game' : hexGame) :
    position * int =
  let game = gameCopy game' in
  let table = Hashtbl.create (truncate (3. ** float game.size)) in
  let auxMoveScoreMMbis (game : hexGame) (curD : int)
      (bestPlay : hexGame -> int -> position * int) ((i, j) : int * int) :
      (int * int) * int =
    play game i j;
    try
  ...
  let rec bestPlay (game : hexGame) (curD : int) : (int * int) * int =
    ...
    if curD > maxDepth then ((-1, -1), heur game)
      ...
      (*pos à bouger dans auxMoveScore*)
      let scoreList =
        List.map (auxMoveScoreMMbis game curD bestPlay) movesList
      in
      List.fold_left
        (if game.curPlayer = 1 then maxCustom else minCustom)
        (if game.curPlayer = 1 then minimumPos else maximumPos)
...
let playGame (out : out_channel) (n : int) (player1 : hexGame -> position * int)
    (player2 : hexGame -> position * int) =
  let game = newGame n in
  let v = ref true in
  let cur = ref 0 in
  while !v do
...
module Position = struct
  ...
  let compare ((i, j) : position) ((k, l) : position) =
    if i - k <> 0 then i - k else j - l
end
...
let iterHashtblCouple (hashtbl : ('a, 'b) Hashtbl.t)
    (f : 'a * 'b -> 'a * 'b -> unit) =
  let aux_iter (ka : 'a) (ela : 'b) : unit =
    let iterbis (kb : 'a) (elb : 'b) : unit = f (ka, ela) (kb, elb) in
    Hashtbl.iter iterbis hashtbl
  in
  Hashtbl.iter aux_iter hashtbl
...
let isNew (i : int) (depth : int) : bool = depth - i <= 1

let areIncl (setList : carrier list) (pset : Posset.t) : bool =
  List.fold_left
    (fun (prev : bool) ((_, pset1) : carrier) : bool ->
      prev || Posset.subset pset1 pset)
    false setList

let rec addOnlyIfMinimal (newc : Posset.t) (odepth : int) (ndepth : int)
    (carl : carrier list) (acc : carrier list) : bool * carrierList =
  match carl with
  | [] -> (true, (max odepth ndepth, (ndepth, newc) :: acc))
  | (d, set) :: carls ->
      if Posset.subset set newc then (false, (odepth, carl @ acc))
      else if Posset.subset newc set then
        addOnlyIfMinimal newc odepth ndepth carls acc
      else addOnlyIfMinimal newc odepth ndepth carls ((d, set) :: acc)

let rec orDeductionRule ((prevd, cSet) : carrierList) (scSet : carrier list)
    (u : Posset.t) (i : Posset.t) (depth : int) (mTH : int) (kTH : int) :
    carrierList =
  if List.length cSet >= mTH || kTH < 0 then (prevd, cSet)
  else
    match scSet with
    | [] -> (prevd, cSet)
    | (_, sc1) :: scs ->
        ...
        if areIncl cSet u1 then
          ...
          let nd, ncset =
            orDeductionRule (prevd, cSet) scs u1 i1 depth mTH (kTH - 1)
          in
...
let incrPoslCount (critic : bool) (posl : posList) (set : Posset.t) : unit =
  ...
  Posset.iter
    (fun position ->
      Hashtbl.replace posl position (Hashtbl.find posl position + incr))
    set

let loop2 (size : int) (posl : posList) (c : cTabl) (sc : scTabl) (empty : bool)
    (g : position) (g1 : position) (g2 : position) (depth : int) (mTH : int)
    (kTH : int) : bool =
  let vmodif = ref false in
  let aux_iterOnCarriers ((d1, c1) : carrier) ((d2, c2) : carrier) : unit =
    if
      (isNew d1 depth || isNew d2 depth)
      && Posset.disjoint c1 c2
      && (not (Posset.mem g1 c2))
      && not (Posset.mem g2 c1)
    then
      if not empty then (
        ...
        if List.length scarl < mTH then
          ...
          if b then (
            ...
            incrPoslCount
              (cpl g1 g1 = cpl (-1, -1) (size, size)
              || cpl g1 g1 = cpl (-1, size) (size, -1))
              posl newsc;
            vmodif := true;
            Hashtbl.replace c (cpl g1 g2)
              (orDeductionRule
                 (Hashtbl.find c (cpl g1 g2))
                 scarl newsc newsc depth mTH kTH))
  in
  let dG1, carListG1 = Hashtbl.find c (cpl g1 g) in
...
let isEmptyPos (game : hexGame) ((i, j) : position) : bool =
  if i < 0 || j < 0 || i >= game.size || j >= game.size then false
  else game.gameSet.(i).(j) = 0

let hsearch (game : hexGame) (g : posList) (c : cTabl) (sc : scTabl) (mTH : int)
    (kTH : int) : unit =
      ...
      (fun pos _ ->
        Hashtbl.iter
          (fun pos1 _ ->
            Hashtbl.iter
              (fun pos2 _ ->
                if List.mem pos [ (-1, -1); (-1, n); (n, -1); (n, n) ] then ()
                else
                  let isE = isEmptyPos game pos in
                  if
                    Position.compare pos1 pos2 > 0
                    (* &&(isE || (isEmptyPos game pos1 || isEmptyPos game pos2))*)
                  then
                    let vmodif =
                      loop2 n g c sc isE pos pos1 pos2 !depth mTH kTH
                    in
                    vstop := !vstop || vmodif)
              g)
          g)
...
let rec remList (l : position list) (x : position) : 'a list =
  ...
  | [] -> []
  | y :: ls ->
      if Position.compare x y = 0 then remList ls x else y :: remList ls x
...
let rec cleanRawNeigh2 (game : hexGame) (l : (int * int) list) :
  ...
  | [] -> []
  | (i, _) :: ls when i < 0 ->
      (-1, n) :: remList (cleanRawNeigh2 game ls) (-1, n)
  | (_, j) :: ls when j < 0 ->
      (-1, -1) :: remList (cleanRawNeigh2 game ls) (-1, -1)
  | (i, _) :: ls when i >= n ->
      (n, -1) :: remList (cleanRawNeigh2 game ls) (n, -1)
...
let generateNeighPile (game : hexGame) (pos : position) : (int * int) list =
  if pos = (-1, -1) then
    let rec auxPileCreate_1 (k : int) (l : (int * int) list) : (int * int) list
        =
      if k = game.size then l else auxPileCreate_1 (k + 1) ((k, 0) :: l)
    in
    auxPileCreate_1 0 []
  else if pos = (-1, game.size) then
    let rec auxPileCreate_2 (k : int) (l : (int * int) list) : (int * int) list
        =
      if k = game.size then l else auxPileCreate_2 (k + 1) ((0, k) :: l)
    in
    auxPileCreate_2 0 []
...
let buildP1EmptyPosList2 (gameSet : int matrix) : posList =
  ...
  for i = 0 to n - 1 do
    for j = 0 to n - 1 do
      if gameSet.(i).(j) >= 0 then Hashtbl.add gpos (i, j) 0
    done
  done;
...
let buildFromGameP1_2 (game : hexGame) : cTabl * scTabl * posList =
  ...
  let aux_fVC ((g1, _) : position * int) ((g2, _) : position * int) : unit =
    if
      Position.compare g1 g2 >= 0
      && Hashtbl.mem posList g1 && Hashtbl.mem posList g2
    then
      if List.mem g2 (rawNeigh2 game g1) then
        Hashtbl.add tblVC (g1, g2) (-1, [ (-1, Posset.empty) ])
      else Hashtbl.add tblVC (g1, g2) (-2, [])
  in
  let aux_fVSC ((g1, _) : position * int) ((g2, _) : position * int) : unit =
    if
      Position.compare g1 g2 >= 0
      && Hashtbl.mem posList g1 && Hashtbl.mem posList g2
    then Hashtbl.add tblVSC (g1, g2) (-2, [])
  in
...
(* 2 dernières etapes :
   ...
   -> Propagation du g, c, sc pour eviter le recalcul complet du H-search *)
...
let resPos (game : hexGame) ((i, j) : position) (player : int) : float =
  let n = game.size in
  let col =
    if indicePos (i, j) n >= n * n then player else game.gameSet.(i).(j)
  in
  if col = player then 0.001 else if col = 0 then 1. else 1000000.
...
let resMat (game : hexGame) (player : int) (vc : cTabl) : float array array =
  ...
  for a = 0 to (n * n) + 1 do
    for b = 0 to (n * n) + 1 do
      ...
      if
        Hashtbl.mem vc (cpl posa posb)
        &&
        let _, list = Hashtbl.find vc (cpl posa posb) in
        list <> []
      then res.(a).(b) <- resPos game posa player +. resPos game posb player
    done
  done;
...
let kirchSystemMatrix (game : hexGame) (player : int) (vc : cTabl) :
    float matrix =
  let n = game.size in
...
let debug_ListDifferencies (vcref : cTabl) (vctest : cTabl) : unit =
  Hashtbl.iter
    (fun couple (_, carL) ->
      ...
      if not (auxListcomp carimage carL) then
        Printf.printf "entre %d,%d et %d,%d \n" i j k l)
    vctest
(* Debug *)
...
let auxUpdateTabl (posPlayed : position) ((nb, setList) : carrierList) :
    carrierList =
  (nb, List.filter (fun (_, set) -> not (Posset.mem posPlayed set)) setList)

let updateHexyTabl (pos : position) (fromPlayer : bool)
    ((vc, vsc, posl) : cTabl * scTabl * posList) : unit =
  if not fromPlayer then (
    Hashtbl.remove posl pos;
    Hashtbl.filter_map_inplace
      (fun (posa, posb) sets ->
        if pos = posa || pos = posb then None else Some (auxUpdateTabl pos sets))
      vc;
    Hashtbl.filter_map_inplace
      (fun (posa, posb) sets ->
        if pos = posa || pos = posb then None else Some (auxUpdateTabl pos sets))
    ...
    Hashtbl.filter_map_inplace (fun _ sets -> Some (auxUpdateTabl pos sets)) vc;
    Hashtbl.filter_map_inplace (fun _ sets -> Some (auxUpdateTabl pos sets)) vsc)

let hexy_noTimeLimit (dTH : int) (mTH : int) (kTH : int) (game' : hexGame) :
    position * int =
  let cpt = ref 0 in
  let game = gameCopy game' in
  let table = Hashtbl.create (truncate (3. ** float game.size)) in
  ...
  let rec bestPlay (game : hexGame) (curD : int) (alpha : int) (beta : int)
      ((vc1b, vsc1b, posl1b) : cTabl * scTabl * posList)
      ((vc2b, vsc2b, posl2b) : cTabl * scTabl * posList) : (int * int) * int =
    let movesList = posMovesList game in
    let auxMoveScore ((vc1b, vsc1b, posl1b) : cTabl * scTabl * posList)
        ((vc2b, vsc2b, posl2b) : cTabl * scTabl * posList) (alpha : int)
        (beta : int) ((i, j) : int * int) : (int * int) * int =
      ...
      with Not_found ->
        (if isWinningPos game 1 then s := max_int
           ...
           let _, s' =
             (* copie ici *)
             bestPlay game (curD + 1) alpha beta (vc1c, vsc1c, posl1c)
               (vc2c, vsc2c, posl2c)
           in
           s := s');
        Hashtbl.add table (hashGame game) !s;
  ...
  in
  let p, s =
    bestPlay game 0 min_int max_int (vc1, vsc1, posl1) (vc2, vsc2, posl2)
  in
  Printf.printf "%d" !cpt;
  (p, s)
...
let posMovesListOpti (n : int) (posOtherPlayer : posList) : position list =
  let posPrioList =
    List.sort
      (fun (_, nb1) (_, nb2) -> nb2 - nb1)
      (Hashtbl.fold (fun pos nb prevl -> (pos, nb) :: prevl) posOtherPlayer [])
  in
  assert (posPrioList <> []);
  let _, max = List.hd posPrioList in
  let posPrioListfiltered =
    List.filter
      (fun ((i, j), nb) ->
        if max >= 1000 then
          nb >= 1000 && inboundPos n i j && nb > 0
          && Float.log10 (float nb /. float max) >= -0.53
        else
          inboundPos n i j && nb > 0
          && Float.log10 (float nb /. float max) >= -0.53)
      posPrioList
  in
...
let decrPoslCount (posl : posList) (set : Posset.t) : unit =
  Posset.iter
    (fun position ->
      try Hashtbl.replace posl position (Hashtbl.find posl position - 1)
      with Not_found -> ())
    set

let auxUpdateTabl2 (posl : posList) (posPlayed : position)
    ((nb, setList) : carrierList) : carrierList =
  ( nb,
    List.filter
      (fun (_, set) ->
...
let updateHexyTabl2 (pos : position) (fromPlayer : bool)
    ((vc, vsc, posl) : cTabl * scTabl * posList) : unit =
  if not fromPlayer then (
    Hashtbl.filter_map_inplace
      (fun (posa, posb) sets ->
    ...
    Hashtbl.filter_map_inplace
      (fun (posa, posb) sets ->
        ...
        else Some (auxUpdateTabl2 posl pos sets))
      vsc;
    Hashtbl.remove posl pos)
  else (
    ...
    Hashtbl.filter_map_inplace (fun _ sets -> Some (auxUpdateTabl pos sets)) vc;
    Hashtbl.filter_map_inplace
      (fun _ sets -> Some (auxUpdateTabl2 posl pos sets))
      vsc)

let hexy_noTLAndMO (dTH : int) (mTH : int) (kTH : int) (game' : hexGame) :
    position * int =
  let cpt = ref 0 in
  let game = gameCopy game' in
  let table = Hashtbl.create (truncate (3. ** float game.size)) in
  ...
  let rec bestPlay (game : hexGame) (curD : int) (alpha : int) (beta : int)
      ...
      ((vc2b, vsc2b, posl2b) : cTabl * scTabl * posList) : (int * int) * int =
    let movesList =
      posMovesListOpti game.size (if game.curPlayer = 1 then posl2b else posl1b)
    in
    let auxMoveScore ((vc1b, vsc1b, posl1b) : cTabl * scTabl * posList)
        ((vc2b, vsc2b, posl2b) : cTabl * scTabl * posList) (alpha : int)
        (beta : int) ((i, j) : int * int) : (int * int) * int =
...
let hexy_wthTL (dTH : int) (mTH : int) (kTH : int) (tLIMIT : float)
    (game' : hexGame) : position * int =
  let t0 = Sys.time () in
  let cpt = ref 0 in
  let game = gameCopy game' in
  let table = Hashtbl.create (truncate (3. ** float game.size)) in
...
(* IDDFS ici *)
let hexy_fullOpti (mTH : int) (kTH : int) (tLIMIT : float) (game : hexGame) :
    position * int =
  let t0 = Sys.time () in
  let p = ref (-2, -2) in
  ...
  while Sys.time () -. t0 < tLIMIT do
    let p', s' =
      hexy_wthTL !dcur mTH kTH (tLIMIT -. (Sys.time () -. t0)) game
    in
    if game.curPlayer = 1 && s' > !s then (
      p := p';
      s := s');
    if game.curPlayer = -1 && s' < !s then (
      p := p';
